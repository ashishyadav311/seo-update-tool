...
Box.JQueryDOM = (function() {
    ...
    return {
		...
		queryData: function(root, dataAttrib, dataValue) {
			return $(root).find('[data-' + dataAttrib + '=' + dataValue + ']')[0] || null;
		},
		...
		queryAllData: function(root, dataAttrib, dataValue) {
			return $.makeArray($(root).find('[data-' + dataAttrib + '=' + dataValue + ']'));
		},
		...
		getModuleConfig: function(element, name) {
			...
			if (instanceData) {
				...
				if (!instanceData.config) {
					...
					if (configElement) {
						innerText = configElement.textContent || configElement.innerText;
						instanceData.config = JSON.parse(innerText);
					}
				}
